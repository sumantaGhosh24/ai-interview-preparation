export const cacheKeys = {
  ...
  questionSourceStats: (userId: string) => `analytics:question-source-stats:${userId}`,

  attemptTrends: (userId: string) => `analytics:attempt-trends:${userId}`,

  topicsList: (userId: string, page: number, pageSize: number, search: string) =>
    `topics:list:${userId}:${page}:${pageSize}:${search}`,

  topicDetail: (userId: string, topicId: string) => `topic:detail:${userId}:${topicId}`,

  questionsList: (topicId: string, page: number, pageSize: number, search: string) =>
